...
3. `save_raw_web_data` — сохраняет сырые данные (source + content) в bank_buffer и возвращает record_ids (передавай название или ID банка/продукта)
...
- Для каждого найденного источника сразу вызывай `save_raw_web_data`, указывая название/ID банка, название/ID продукта, URL и текст страницы. Этот инструмент вернет JSON с `record_ids`
